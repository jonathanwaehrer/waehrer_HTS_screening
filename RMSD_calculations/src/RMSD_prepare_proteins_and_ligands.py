...
def prepare_ligands_for_vina(bin_dir: str, path_to_ligands: str):
    """
    ...
    ----------
    bin_dir: Binary executables.
    path_to_ligands: Directory containing .mol2 files of initial protein ligands
    """
    # ---- Get file path of output directory and copy initial ligands ---- #
    current_dir = os.path.dirname(os.path.realpath(__file__))
    ligand_dir = current_dir + "/../out/prepared_ligands"
    if not os.path.exists(ligand_dir):
        os.makedirs(ligand_dir, exist_ok=True)
    os.system("cp %s/*.mol2 %s/" % (path_to_ligands, ligand_dir))
    bin_dir = os.path.dirname(bin_dir)
...
# ==================================================================================================================== #
def run(input_proteins, path_to_ligands, path_to_bin, system="mac"):
    prepare_proteins(pdb_dir=input_proteins, bin_dir=path_to_bin, system=system)
    prepare_ligands_for_vina(bin_dir=path_to_bin, path_to_ligands=path_to_ligands)
